...
const part2 = (rawInput) => {
  ...
  const games = input.split("\n").filter((n) => n).map((line) => {
    ...
    return game.tries.flat().reduce((acc, obj) => {
      const count = parseInt(obj.count); // Convert count to number
      if (!acc[obj.color] || count > acc[obj.color]) {
        acc[obj.color] = count;
      }
      return acc;
    }, {})
  })
  const total= games.reduce((accumulator, currentObject) => accumulator + (currentObject.red * currentObject.blue * currentObject.green), 0);
  return total
}
